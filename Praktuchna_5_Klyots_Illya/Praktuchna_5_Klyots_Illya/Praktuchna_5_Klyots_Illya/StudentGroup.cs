...
namespace Praktuchna_5_Klyots_Illya
{
    internal class StudentGroup
    {
        ...
        {
            if (members.Count == 0) return "Група порожня.";

            int totalLetters = members.Sum(m => m.FullName.Replace(" ", "").Length);

            return $"Аналітика: Записів - {members.Count}, Всього літер у ПІБ - {totalLetters}";
        }
    }
}
