...
namespace Praktuchna_4_Klyots_Illya
{
    internal class StudentGroup
    {
        ...
        {
            get => this.students.FirstOrDefault(s => s.RecordBookNumber == recordBookNumber);
        }
        ...
        public static StudentGroup operator +(StudentGroup a, StudentGroup b)
        {
            ...
            foreach (var s in a.students) newGroup.AddStudent(s);
            foreach (var s in b.students) newGroup.AddStudent(s);
            return newGroup;
        }

        // Метод для пошуку найкращого студента
        public Student BestStudent()
        {
            if (!this.students.Any()) return null;
