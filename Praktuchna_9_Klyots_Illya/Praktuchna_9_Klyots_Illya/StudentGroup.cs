...
namespace Praktuchna_9_Klyots_Illya
{
    public class StudentGroup
    {
        ...
        public List<Student> TargetStudents => members.OfType<Student>().ToList();

        // Практична 9
        public List<Student> FilterStudents(Predicate<Student> predicate)
        {
            return GetAllStudents().FindAll(predicate);
        }

        public void PerformOperationOnStudents(Func<Student, bool> filterPredicate, Action<Student> action)
        {
            foreach (var student in GetAllStudents())
            {
                if (filterPredicate(student))
                {
                    action(student);
                }
            }
        ...
        public void AddMember(UniversityMember member)
        {
            ...
            {
                student.AverageGradeChanged += (sender, e) =>
                {
                    GradeChanged?.Invoke(this, e);
                };

                StudentAdded?.Invoke(this, new StudentEventArgs(student));
                EventManager.RaiseStudentAdded(this, student);
            }
        }
        ...
        public void RemoveStudent(string recordBookNumber)
        {
            ...
            if (student != null)
            {
                ...
                EventManager.RaiseStudentRemoved(this, student);
        ...
        public void SaveToFile(string fileName)
        {
            try
            {
                ...
                fm.SaveToJson(this.GetAllStudents(), fileName);
        ...
        public void LoadFromFile(string fileName)
        {
            try
            {
                ...
                {
                    FileManager fm = new FileManager();
                    var loadedStudents = fm.LoadFromJson<List<Student>>(fileName);

                    members.Clear();
                    foreach (var s in loadedStudents) members.Add(s);
        ...
        // Методи для практичної 8
        public void Save(string filePath, StorageFormat format)
        {
            FileManager fm = new FileManager();
            if (format == StorageFormat.Json)
            {
                fm.SaveToJson(this.GetAllStudents(), filePath); 
            }
            else
            {
                fm.SaveToText(this.SearchByNameFragment(""), filePath);
            }
        }
        ...
        public static StudentGroup Load(string filePath, StorageFormat format)
        {
            ...
            if (format == StorageFormat.Json)
            {
                var loadedStudents = fm.LoadFromJson<List<Student>>(filePath);
                foreach (var s in loadedStudents)
                {
                    newGroup.AddStudent(s);
                }
                return newGroup;
            }
            throw new InvalidFileFormatException("Завантаження через TXT не підтримується.");
        }
    ...
    public enum StorageFormat { Json, Txt }
}